 ...
 * @author Jesu
 */
public class FiltroSobel3x3 extends ConvolucionImagen implements KernelConvolucion{
    private int[][] matrizEscalaDeGrises;
    
    public FiltroSobel3x3(BufferedImage imagen){    
        super(imagen);
        this.matrizEscalaDeGrises = obtenerEscalaGris();
    }
    
    public FiltroSobel3x3(String rutaString) {
        super(rutaString);
        this.matrizEscalaDeGrises = obtenerEscalaGris();
    }
    ...
    public BufferedImage deteccionBordes(int gradiente) {
        ...
        cargarMatrizConvolucion(SOBEL3x3_GX);
        int[][] Mx = generarConvolucion(getMatrizEscalaDeGrises());
        cargarMatrizConvolucion(SOBEL3x3_GY);
            ...
            for (int x = ext/2; x < getAncho() + (ext/2); x++) {
                ...
                for (int i = 0; i < n; i++) {
                    for (int j = 0; j < n; j++) {
                        ...
                        sumaProd += (valorRGB&0xFF) * getMatrizConvolucion()[i][j];
                    }
                }
